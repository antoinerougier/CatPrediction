...
def get_dataloaders(data_dir="data", batch_size=BATCH_SIZE):
    train_dataset = datasets.ImageFolder(
        f"{data_dir}/train", transform=get_transforms(train=True)
    )
    val_dataset = datasets.ImageFolder(
        f"{data_dir}/val", transform=get_transforms(train=False)
    )
    test_dataset = datasets.ImageFolder(
        f"{data_dir}/test", transform=get_transforms(train=False)
    )
